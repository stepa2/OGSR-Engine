...
const CPatrolPath* CPatrolPathStorage::safe_path(shared_str patrol_name, bool no_assert, bool on_level) const
{
	...
	for (auto& it2 : (*it).second->vertices())
	{
		...
		if (on_level || (ai().game_graph().valid_vertex_id(pp.m_game_vertex_id) && ai()
		                                                                           .game_graph().vertex(
			                                                                           pp.m_game_vertex_id)->level_id()
			== ai().level_graph().level_id()))
		{
			if (!ai().level_graph().valid_vertex_id(pp.m_level_vertex_id))
			{
				...
				Msg("* [%s]: path[%s] pp[%s] level_vertex_id[%u] -> %u", __FUNCTION__, patrol_name.c_str(),
				    pp.m_name.c_str(), prev_vertex_id, pp.m_level_vertex_id);
			}
		}
		else
			return path(patrol_name, no_assert);
