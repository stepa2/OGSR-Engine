...
void CSpaceRestrictor::OnRender()
{
	...
	if (Device.vCameraPosition.distance_to(XFORM().c) < 100.0f)
	{
		...
		if (z)
		{
			...
			switch (z->ZoneState())
			{
			case CCustomZone::eZoneStateIdle: strcpy(str, "IDLE");
				break;
			case CCustomZone::eZoneStateAwaking: strcpy(str, "AWAKING");
				break;
			case CCustomZone::eZoneStateBlowout: strcpy(str, "BLOWOUT");
				break;
			case CCustomZone::eZoneStateAccumulate: strcpy(str, "ACCUMULATE");
				break;
			case CCustomZone::eZoneStateDisabled: strcpy(str, "DISABLED");
				break;
			};
