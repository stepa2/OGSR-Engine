...
void CTorch::SetColor(const Fcolor& color, int target)
{
	...
	switch (target)
	{
	case 0: light_render->set_color(c);
		break;
	case 1: if (light_omni) light_omni->set_color(c);
		break;
	case 2: if (glow_render) glow_render->set_color(c);
		break;
	}
}
...
void CTorch::SetAngle(float angle, int target)
{
	switch (target)
	{
	case 0: light_render->set_cone(angle);
		break;
	case 1: if (light_omni) light_omni->set_cone(angle);
		break;
	}
}
...
void CTorch::SetRange(float range, int target)
{
	switch (target)
	{
	case 0: light_render->set_range(range);
		break;
	case 1: if (light_omni) light_omni->set_range(range);
		break;
	case 2: if (glow_render) glow_render->set_radius(range);
		break;
	}
}
...
void CTorch::SetTexture(LPCSTR texture, int target)
{
	switch (target)
	{
	case 0: light_render->set_texture(texture);
		break;
	case 1: if (light_omni) light_omni->set_texture(texture);
		break;
	case 2: if (glow_render) glow_render->set_texture(texture);
		break;
	}
}
