	...
	else
	{
		     ...
		     it++)
		{
			...
			if (next_phrase_vertex->data()->m_PhraseScript.Precondition(pSpeakerGO2, pSpeakerGO1,
			                                                            *phrase_dialog->m_DialogId, phrase_id.c_str(),
...
LPCSTR CPhraseDialog::GetPhraseText(const shared_str& phrase_id, bool current_speaking)
{
	...
	return ph->m_PhraseScript.GetScriptText(ph->GetText(), pSpeakerGO1, pSpeakerGO2, m_DialogId.c_str(),
	                                        phrase_id.c_str());
}
...
void CPhraseDialog::Load(shared_str dialog_id)
{
	...
	if (GetForceReload())
	{
		if (data()->m_sInitFunction.size())
		{
			...
			ASSERT_FMT_DBG(functor_exists, "!![%s] Cannot find precondition [%s]", __FUNCTION__,
			               data()->m_sInitFunction.c_str());
			if (functor_exists)
				lua_function(this);
		}
	}
