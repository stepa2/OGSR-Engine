...
public:
	...
	dBodyID GetBody()
	{
		if (m_character) return m_character->get_body();
		else return NULL;
	}
	...
	{
		if (m_character)m_character->GetVelocity(velocity);
		else velocity.set(0.f, 0.f, 0.f);
	}
	...
	{
		if (m_character)m_character->GetSmothedVelocity(v);
		else v.set(0, 0, 0);
	}
	...
	float FootRadius()
	{
		if (m_character)return m_character->FootRadius();
		else return 0.f;
	};

	void CollisionEnable(BOOL enable)
	{
		if (m_character) if (enable)m_character->collision_enable();
		else m_character->collision_disable();
	}
	...
	{
		if (m_character)return m_character->ObjectContactCallBack();
		else return NULL;
	}
