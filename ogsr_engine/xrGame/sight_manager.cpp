...
{
	MonsterSpace::SBoneRotation orientation = object().movement().m_head, body_orientation = object()
	                                                                                         .movement().
	                                                                                         body_orientation();
...
bool CSightManager::bfIf_I_SeePosition(Fvector tPosition) const
{
	...
	return (angle_difference(yaw, object().movement().m_head.current.yaw) <= PI_DIV_6);
	// && angle_difference(pitch,object().movement().m_head.current.pitch,PI_DIV_6));
}
