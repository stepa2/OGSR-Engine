...
#pragma todo("KRodin: подумать над значениями по умолчанию. Сейчас по дефолту всё выключено.")
		m_zone_flags.set(eIdleLightVolumetric,
		                 READ_IF_EXISTS(pSettings, r_bool, section, "idle_light_volumetric", false));
	...
	if (m_zone_flags.test(eSpawnBlowoutArtefacts))
	{
		...
		m_fArtefactSpawnOnDeathProbability = READ_IF_EXISTS(pSettings, r_float, section, "birth_on_death_probability",
		                                                    0.0f);
