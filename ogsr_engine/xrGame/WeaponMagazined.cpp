...
BOOL CWeaponMagazined::net_Spawn(CSE_Abstract* DC)
{
	...
	{
		Msg("! [%s]: %s: wrong m_iCurFireMode[%u/%u]", __FUNCTION__, cName().c_str(), m_iCurFireMode,
		    m_aFireModes.size() - 1);
...
void CWeaponMagazined::Load(LPCSTR section)
{
	...
	animGetEx(mhud.mhud_idle_moving,
	          pSettings->line_exist(hud_sect.c_str(), "anim_idle_moving") ? "anim_idle_moving" : "anim_idle");
	animGetEx(mhud.mhud_idle_sprint,
	          pSettings->line_exist(hud_sect.c_str(), "anim_idle_sprint") ? "anim_idle_sprint" : "anim_idle");
		...
		if (m_pAmmo || unlimited_ammo() || (IsMisfire() && iAmmoElapsed))
		{
			...
			for (u32 i = 0; i < m_ammoTypes.size(); ++i)
			{
				...
				if (m_pAmmo)
				{
					...
					// https://github.com/revolucas/CoC-Xray/pull/5/commits/3c45cad1edb388664efbe3bb20a29f92e2d827ca
					m_bPending = true;
					SwitchState(eReload);
					return true;
				}
...
void CWeaponMagazined::ReloadMagazine()
{
	...
	if (Core.Features.test(xrCore::Feature::hard_ammo_reload))
	{
		if (!m_bLockType && !m_magazine.empty())
			if ((ParentIsActor() && !unlimited_ammo()) || (!m_pAmmo || xr_strcmp(
				m_pAmmo->cNameSect(), *m_magazine.back().m_ammoSect)))
				UnloadMagazine();
	}
	else
	{
...
void CWeaponMagazined::state_Fire(float dt)
{
	...
	while (!m_magazine.empty() && fTime <= 0 && (IsWorking() || m_bFireSingleShot) && (m_iQueueSize < 0 || m_iShotNum <
		m_iQueueSize))
	{
		m_bFireSingleShot = false;
...
bool CWeaponMagazined::Action(s32 cmd, u32 flags)
{
	...
	case kWPN_RELOAD:
		{
			if (!Core.Features.test(xrCore::Feature::lock_reload_in_sprint) || (!ParentIsActor() || !(g_actor->
				get_state() & mcSprint)))
				if (flags & CMD_START)
...
void CWeaponMagazined::InitZoomParams(LPCSTR section, bool useTexture)
{
	...
	if (dynamicZoomParams)
	{
		...
		ASSERT_FMT(num_zoom_param >= 1, "!![%s] : Invalid scope_dynamic_zoom parameter in section [%s]", __FUNCTION__,
		           section);
	...
	m_fScopeInertionFactor = READ_IF_EXISTS(pSettings, r_float, section, "scope_inertion_factor",
	...
	if (useTexture)
	{
		...
		if (scope_tex_name.size() > 0)
		{
			...
			m_UIScope->Init(scope_tex_name.c_str(),
			                Core.Features.test(xrCore::Feature::scope_textures_autoresize)
				                ? "hud\\scope"
				                : "hud\\default", 0, 0, alNone);
		}
	}
}
...
void CWeaponMagazined::InitAddons()
{
	...
	m_fSecondScopeZoomFactor = READ_IF_EXISTS(pSettings, r_float, cNameSect(), "second_scope_zoom_factor",
	                                          m_fIronSightZoomFactor);
	...
	if (IsScopeAttached())
	{
		if (m_eScopeStatus == ALife::eAddonAttachable)
		{
			...
			m_fZoomHudFov = READ_IF_EXISTS(pSettings, r_float, cNameSect().c_str(), "scope_zoom_hud_fov",
			                               m_fZoomHudFov);
			m_fSecondVPHudFov = READ_IF_EXISTS(pSettings, r_float, cNameSect().c_str(), "scope_lense_hud_fov",
			                                   m_fSecondVPHudFov);
		}
	...
	else
	{
		...
		if (IsZoomEnabled())
		{
			InitZoomParams(cNameSect().c_str(),
			               !!READ_IF_EXISTS(pSettings, r_bool, cNameSect(), "force_scope_texture", false));
...
{
	if (!m_bHasDifferentFireModes || m_iPrefferedFireMode == -1 || u32(GetCurrentFireMode()) <= u32(
		m_iPrefferedFireMode))
...
void CWeaponMagazined::GetBriefInfo(xr_string& str_name, xr_string& icon_sect_name, xr_string& str_count)
{
	...
	string256 sItemName;
	strcpy_s(sItemName,
	         CStringTable().translate(pSettings->r_string(icon_sect_name.c_str(), "inv_name_short")).c_str());
...
bool CWeaponMagazined::ScopeRespawn(PIItem pIItem)
{
	...
	if (pSettings->line_exist(cNameSect(), scope_respawn.c_str()))
	{
		...
		{
			CSE_Abstract* _abstract = Level().spawn_item(S, Position(), ai_location().level_vertex_id(),
			                                             H_Parent()->ID(), true);
