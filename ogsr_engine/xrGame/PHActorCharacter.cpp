...
void SPHCharacterRestrictor::Create(CPHCharacter* ch, dVector3 sizes)
{
	...
	case CPHCharacter::rtStalker: static_cast<CPHActorCharacter::stalker_restrictor*>(this)->Create(ch, sizes);
		break;
	case CPHCharacter::rtStalkerSmall: static_cast<CPHActorCharacter::stalker_small_restrictor*>(this)->
			Create(ch, sizes);
		break;
	case CPHCharacter::rtMonsterMedium: static_cast<CPHActorCharacter::medium_monster_restrictor*>(this)->
			Create(ch, sizes);
		break;
	default: NODEFAULT;
	}
...
void CPHActorCharacter::ChooseRestrictionType(CPHCharacter::ERestrictionType my_type, float my_depth, CPHCharacter* ch)
{
	...
	float checkR = m_restrictors[rtStalkerSmall]->m_restrictor_radius * 1.5f;
	//+m_restrictors[rtStalker]->m_restrictor_radius)/2.f;
