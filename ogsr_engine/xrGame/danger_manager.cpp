	...
	if (obj && !obj->g_Alive() && (obj->killer_id() != ALife::_OBJECT_ID(-1)))
	{
		if (!on_before_add.empty())
		{
			...
			if (ai().script_engine().functor(on_before_add.c_str(), funct))
				if (!funct(m_object->lua_game_object(), object, obj->Position(), object.m_level_time,
				           CDangerObject::eDangerTypeFreshEntityCorpse, CDangerObject::eDangerPerceiveTypeVisual))
					return;
		}
...
void CDangerManager::add(const CSoundObject& object)
{
	...
	if ((object.m_sound_type & SOUND_TYPE_BULLET_HIT) == SOUND_TYPE_BULLET_HIT)
	{
		if (!on_before_add.empty())
		{
			luabind::functor<bool> funct;
			if (ai().script_engine().functor(on_before_add.c_str(), funct))
				if (!funct(m_object->lua_game_object(), object, object.m_object_params.m_position, object.m_level_time,
	...
	if ((object.m_sound_type & SOUND_TYPE_INJURING) == SOUND_TYPE_INJURING)
	{
		...
		if (do_add)
		{
			if (!on_before_add.empty())
			{
				luabind::functor<bool> funct;
				if (ai().script_engine().functor(on_before_add.c_str(), funct))
					if (!funct(m_object->lua_game_object(), object, object.m_object_params.m_position,
					           object.m_level_time, CDangerObject::eDangerTypeEntityAttacked,
...
void CDangerManager::add(const CHitObject& object)
{
	...
	if (!on_before_add.empty())
	{
		...
		if (ai().script_engine().functor(on_before_add.c_str(), funct))
			if (!funct(m_object->lua_game_object(), object, obj->Position(), object.m_level_time,
			           CDangerObject::eDangerTypeAttacked, CDangerObject::eDangerPerceiveTypeHit))
				return;
	}
