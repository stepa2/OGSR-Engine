...
void NodynamicsCollide(bool& do_colide, bool bo1, dContact& c, SGameMtl* /*material_1*/, SGameMtl* /*material_2*/)
{
	...
	if (!body1 || !body2 || (dGeomUserDataHasCallback(c.geom.g1, NodynamicsCollide) && dGeomUserDataHasCallback(
		c.geom.g2, NodynamicsCollide)))return;
	do_colide = false;
}
...
#pragma todo("KRodin: V621 Consider inspecting the 'for' operator. It's possible that the loop will be executed incorrectly or won't be executed at all.")
	...
	if (anim_mov_ctrl && anim_mov_blend && anim_mov_blend->blend_state() != CBlend::eFREE_SLOT && anim_mov_blend->
		timeCurrent + Device.fTimeDelta * anim_mov_blend->speed < anim_mov_blend->timeTotal - SAMPLE_SPF - EPS) //.
	{
