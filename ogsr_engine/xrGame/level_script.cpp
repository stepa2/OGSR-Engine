...
void set_time_factor(float time_factor)
{
	...
	GamePersistent().Environment().SetGameTime(Level().GetEnvironmentGameDayTimeSec(),
	                                           Level().game->GetEnvironmentGameTimeFactor());
}
...
script_rq_result PerformRayQuery(Fvector start, Fvector dir, float range, collide::rq_target tgt,
                                 CScriptGameObject* ignore)
{
...
CPHCall* add_call(const luabind::object& lua_object, const luabind::functor<bool>& condition,
...
void AdvanceGameTime(u32 _ms)
{
	...
	Level().game->SetGameTimeFactor(ai().get_alife() ? ai().alife().time().game_time() : Level().GetGameTime(),
	                                Level().game->GetGameTimeFactor());
	GamePersistent().Environment().SetGameTime(Level().GetEnvironmentGameDayTimeSec(),
	                                           Level().game->GetEnvironmentGameTimeFactor());
}
	...
	if (partially_inside)
		return (
			CSpaceRestrictionBase_inside(
				m_sphere, construct_position(level_vertex_id, position.x + offset, position.z + offset), radius) ||
			CSpaceRestrictionBase_inside(
				m_sphere, construct_position(level_vertex_id, position.x + offset, position.z - offset), radius) ||
			CSpaceRestrictionBase_inside(
				m_sphere, construct_position(level_vertex_id, position.x - offset, position.z + offset), radius) ||
...
void CLevel::script_register(lua_State* L)
{
	module(L)
		...
		def("check_object",						check_object),
			...
			def("add_call", ((CPHCall* (*)(const luabind::object&, const luabind::functor<bool>&,
