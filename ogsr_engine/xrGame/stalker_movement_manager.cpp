...
void CStalkerMovementManager::setup_movement_params()
{
	...
	{
		if ((path_type() != MovementManager::ePathTypePatrolPath) && (path_type() != MovementManager::ePathTypeGamePath)
			&& (path_type() != MovementManager::ePathTypeNoPath))
		{
...
void CStalkerMovementManager::parse_velocity_mask()
{
	...
	if ((movement_type() == eMovementTypeStand) || path().empty() || (path().size() <= detail().
		curr_travel_point_index()) || path_completed() || !actual())
	{
