...
CWeaponMagazinedWGrenade::CWeaponMagazinedWGrenade(LPCSTR name, ESoundTypes eSoundType) : CWeaponMagazined(
	name, eSoundType)
{
	m_ammoType2 = 0;
	iAmmoElapsed2 = 0;
...
void CWeaponMagazinedWGrenade::Load(LPCSTR section)
{
	...
	animGetEx(mhud_idle_moving_g,
	          pSettings->line_exist(hud_sect.c_str(), "anim_idle_moving_g") ? "anim_idle_moving_g" : "anim_idle_g");
	animGetEx(mhud_idle_sprint_g, pSettings->line_exist(hud_sect.c_str(), "anim_idle_sprint_g")
		                              ? "anim_idle_sprint_g"
		                              : pSettings->line_exist(hud_sect.c_str(), "anim_idle_sprint")
		                              ? "anim_idle_sprint"
		                              : "anim_idle_g");
	...
	animGetEx(mhud_idle_moving_gl, pSettings->line_exist(hud_sect.c_str(), "anim_idle_moving_gl")
		                               ? "anim_idle_moving_gl"
		                               : "anim_idle_gl");
	animGetEx(mhud_idle_sprint_gl, pSettings->line_exist(hud_sect.c_str(), "anim_idle_sprint_gl")
		                               ? "anim_idle_sprint_gl"
		                               : pSettings->line_exist(hud_sect.c_str(), "anim_idle_sprint")
		                               ? "anim_idle_sprint"
		                               : "anim_idle_gl");
...
BOOL CWeaponMagazinedWGrenade::net_Spawn(CSE_Abstract* DC)
{
	...
	{
		ASSERT_FMT(m_ammoType2 < m_ammoTypes2.size(), "Ammo type [%u] not found in weapon [%s]. Something strange...",
		           m_ammoType2, this->cName().c_str());
...
void CWeaponMagazinedWGrenade::PerformSwitchGL()
{
	...
	// https://github.com/revolucas/CoC-Xray/pull/5/commits/4a396eb30137c5625c5b0dd934e63eaa5b62cbc5
...
bool CWeaponMagazinedWGrenade::TryPlayAnimIdle(u8 state = eIdle)
{
	...
	if (IsGrenadeLauncherAttached() && !IsZoomed())
	{
		...
		case eSubstateIdleMoving:
			m_pHUD->animPlay(random_anim(m_bGrenadeMode ? mhud_idle_moving_g : mhud_idle_moving_gl), TRUE, NULL,
			                 GetState());
			return true;
		case eSubstateIdleSprint:
			m_pHUD->animPlay(random_anim(m_bGrenadeMode ? mhud_idle_sprint_g : mhud_idle_sprint_gl), TRUE, NULL,
			                 GetState());
			return true;
		default:
			return false;
...
//Этот и все подобные методы вообще не вызываются в в синглплеере, походу.
{
	u8 _data = P.r_u8();
