...
void CCar::RestoreNetState(CSE_PHSkeleton* /*po*/)
{
	...
	if (co->door_states.size() == m_doors.size())
	{
		...
		*/
		...
		Msg("~ [%s]: [%s] has different state in m_doors[%u] door_states[%u] Visual[%s]", __FUNCTION__,
		    obj->Name_script(), m_doors.size(), co->door_states.size(), obj->cNameVisual().c_str());
	...
	if (co->wheel_states.size() == m_wheels_map.size())
	{
		...
		*/
		...
		Msg("~ [%s]: [%s] has different state in m_wheels_map[%u] wheel_states[%u] Visual[%s]", __FUNCTION__,
		    obj->Name_script(), m_wheels_map.size(), co->wheel_states.size(), obj->cNameVisual().c_str());
