...
void CActorCondition::UpdateCondition()
{
	...
	{
		ConditionWalk(weight_coef, isActorAccelerated(object().mstate_real, object().IsZoomAimingMode()),
...
void CActorCondition::UpdateSatiety()
{
	...
	if (m_fSatietyLightLimit > 0)
	{
		if (m_fSatiety < m_fSatietyLightLimit)
		{
			...
			satiety_health_koef = (m_fSatiety / m_fSatietyLightLimit - critical_k) / (
				m_fSatiety >= m_fSatietyCriticalLimit ? 1 - critical_k : critical_k);
		}
	}
	else
	{
...
void CActorCondition::UpdateThirst()
{
	...
	if (m_fThirstLightLimit > 0)
	{
		if (m_fThirst < m_fThirstLightLimit)
		{
			...
			thirst_health_koef = (m_fThirst / m_fThirstLightLimit - critical_k) / (m_fThirst >= m_fThirstCriticalLimit
				                                                                       ? 1 - critical_k
				                                                                       : critical_k);
		}
	}
