...
const CCoverPoint* CScriptGameObject::ambush_cover(const Fvector& position, const Fvector& enemy_position, float radius,
                                                   float min_distance)
{
	CAI_Stalker* stalker = smart_cast<CAI_Stalker*>(&object());
	ASSERT_FMT(stalker, "[%s]: %s not a CAI_Stalker", __FUNCTION__, object().cName().c_str());
...
const CCoverPoint* CScriptGameObject::ambush_cover(const Fvector& position, const Fvector& enemy_position, float radius,
                                                   float min_distance, const luabind::functor<bool>& callback)
{
	CAI_Stalker* stalker = smart_cast<CAI_Stalker*>(&object());
	ASSERT_FMT(stalker, "[%s]: %s not a CAI_Stalker", __FUNCTION__, object().cName().c_str());
	std::vector<const CCoverPoint*> covers;
...
CScriptGameObject* CScriptGameObject::GetEnemy() const
{
	...
	{
		if (l_tpCustomMonster->GetCurrentEnemy() && !l_tpCustomMonster->GetCurrentEnemy()->getDestroy()) return (
...
CScriptGameObject* CScriptGameObject::GetCorpse() const
{
	CCustomMonster* l_tpCustomMonster = smart_cast<CCustomMonster*>(&object());
	if (l_tpCustomMonster)
		if (l_tpCustomMonster->GetCurrentCorpse() && !l_tpCustomMonster->GetCurrentCorpse()->getDestroy()) return (
...
CScriptGameObject* CScriptGameObject::GetCurrentOutfit() const
{
	...
	CGameObject* current_equipment = inventoryOwner->GetCurrentOutfit()
		                                 ? &inventoryOwner->GetCurrentOutfit()->object()
		                                 : 0;
...
LPCSTR CScriptGameObject::GetPatrolPathName()
{
	...
	if (!stalker)
	{
		...
		if (!script_monster)
		{
			Msg("!![%s] cannot access class member GetPatrolPathName! Object: [%s]", __FUNCTION__,
			    this->cName().c_str());
			return ("");
		}
		else
...
void CScriptGameObject::set_desired_position(const Fvector* desired_position)
{
	...
	{
		THROW2										(desired_position || stalker->movement().restrictions().accessible(*desired_position),
		                                             *stalker->cName());
...
CScriptGameObject* CScriptGameObject::GetActiveItem()
{
	...
	{
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
		                                "CScriptGameObject : cannot access class member active_item!");
		return (0);
	}
}
...
void CScriptGameObject::SetMaxWeight(float _weight)
{
	...
	{
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
		                                "CActor : cannot access class member SetMaxWeight!");
		return;
	}
...
void CScriptGameObject::SetMaxWalkWeight(float _weight)
{
	...
	{
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
		                                "CActor : cannot access class member SetMaxWalkWeight!");
		return;
	}
...
u32 CScriptGameObject::CalcItemPrice(CScriptGameObject* item, bool b_buying) const
{
	...
	{
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
		                                "CScriptGameObject::CalcItemPrice non-CInventoryItem object !!!");
		return 0;
	}
...
const CCoverPoint* CScriptGameObject::angle_cover(const Fvector& position, float radius, const Fvector& enemy_position,
                                                  float min_enemy_distance, float max_enemy_distance,
                                                  u32 enemy_vertex_id)
{
	CAI_Stalker* stalker = smart_cast<CAI_Stalker*>(&object());
