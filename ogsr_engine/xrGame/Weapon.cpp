...
void CWeapon::Load(LPCSTR section)
{
	...
	m_bZoomInertionAllow = READ_IF_EXISTS(pSettings, r_bool, hud_sect, "allow_zoom_inertion",
	                                      READ_IF_EXISTS(pSettings, r_bool, "features", "default_allow_zoom_inertion",
		                                      true));
	...
	m_nearwall_on = READ_IF_EXISTS(pSettings, r_bool, section, "nearwall_on",
	                               READ_IF_EXISTS(pSettings, r_bool, "features", "default_nearwall_on", true));
	if (m_nearwall_on)
	{
	...
	// Смещение в стрейфе
	m_strafe_offset[0][0] = READ_IF_EXISTS(pSettings, r_fvector3, section,
	                                       xr_strconcat(val_name, "strafe_hud_offset_pos", _prefix),
	                                       Fvector().set(0.015f, 0.f, 0.f));
	m_strafe_offset[1][0] = READ_IF_EXISTS(pSettings, r_fvector3, section,
	                                       xr_strconcat(val_name, "strafe_hud_offset_rot", _prefix),
	                                       Fvector().set(0.f, 0.f, 4.5f));

	// Поворот в стрейфе
	m_strafe_offset[0][1] = READ_IF_EXISTS(pSettings, r_fvector3, section,
	                                       xr_strconcat(val_name, "strafe_aim_hud_offset_pos", _prefix),
	                                       Fvector().set(0.005f, 0.f, 0.f));
	m_strafe_offset[1][1] = READ_IF_EXISTS(pSettings, r_fvector3, section,
	                                       xr_strconcat(val_name, "strafe_aim_hud_offset_rot", _prefix),
	                                       Fvector().set(0.f, 0.f, 2.5f));
	...
	bool bStrafeEnabled = READ_IF_EXISTS(pSettings, r_bool, section, "strafe_enabled",
	                                     READ_IF_EXISTS(pSettings, r_bool, "features", "default_strafe_enabled", true));
...
void CWeapon::LoadZoomOffset(LPCSTR section, LPCSTR prefix)
{
	...
	if (Core.Features.test(xrCore::Feature::ogse_wpn_zoom_system)
		...
		&& !READ_IF_EXISTS(pSettings, r_bool, *cNameSect(), "disable_second_scope", false)
		//И второй прицел не запрещён (нужно для поддержки замороченной системы ogse_addons)
		&& pSettings->line_exist(hud_sect, strconcat(sizeof(full_name), full_name, "second_", prefix, "zoom_offset"))
		//И в секциии худа есть настройки для второго режима прицеливания
	...
	)
	{
		...
		m_pHUD->SetZoomOffset(
			pSettings->r_fvector3(hud_sect, strconcat(sizeof(full_name), full_name, "second_", prefix, "zoom_offset")));
		m_pHUD->SetZoomRotateX(
			pSettings->r_float(hud_sect, strconcat(sizeof(full_name), full_name, "second_", prefix, "zoom_rotate_x")));
		m_pHUD->SetZoomRotateY(
			pSettings->r_float(hud_sect, strconcat(sizeof(full_name), full_name, "second_", prefix, "zoom_rotate_y")));
		//
	...
	callback(GameObject::eOnSecondScopeSwitch)(is_second_zoom_offset_enabled);
	//Для нормальной поддержки скриптовых оружейных наворотов ОГСЕ
}
...
#pragma todo("KRodin: адаптировать тепловизор и тп. под новый рендер, если это возможно.")
	...
	if (!IsHidden())
	{
		...
		if (psActorFlags.test(AF_DOF_SCOPE) && !(IsZoomed() && !IsRotatingToZoom() && (IsScopeAttached() ||
			m_bForceScopeDOF) && !IsGrenadeMode() && m_bUseScopeDOF))
			w_states.x = 0.f;
...
u8 CWeapon::idle_state()
{
	...
	if (actor)
		...
		{
			if (actor->is_actor_running() || actor->is_actor_walking() || actor->is_actor_creeping() || actor->
				is_actor_crouching())
	...
	case kWPN_NEXT:
		{
			...
			if (Core.Features.test(xrCore::Feature::lock_reload_in_sprint) && ParentIsActor() && g_actor->get_state() &
				mcSprint)
				return true;
			...
			if (flags & CMD_START)
			{
				...
				do
				{
					...
					b2 = unlimited_ammo()
						     ? false
						     : (!m_pCurrentInventory->GetAmmo(*m_ammoTypes[l_newType], ParentIsActor()));
				}
				while (b1 && b2);
	...
	case kWPN_ZOOM_DEC:
		{
			if (IsZoomEnabled() && IsZoomed() && m_bScopeDynamicZoom && IsScopeAttached() && !
				is_second_zoom_offset_enabled && (flags & CMD_START))
			{
...
int CWeapon::GetAmmoCurrent(bool use_item_to_spawn) const
{
	...
	for (int i = 0; i < (int)m_ammoTypes.size(); ++i)
	{
		...
		for (TIItemContainer::iterator l_it = m_pCurrentInventory->m_belt.begin(); m_pCurrentInventory->m_belt.end() !=
		     l_it; ++l_it)
		{
...
void CWeapon::UpdateHUDAddonsVisibility()
{
	...
	if (ScopeAttachable())
	{
		ASSERT_FMT_DBG(bone_id != BI_NONE, "[%s] invalid scope bone in section [%s]", __FUNCTION__,
		               this->cNameSect().c_str());
		if (IsScopeAttached())
		{
	...
	{
		ASSERT_FMT_DBG(bone_id != BI_NONE, "[%s] invalid silencer bone in section [%s]", __FUNCTION__,
		               this->cNameSect().c_str());
	...
	{
		if (bone_id == BI_NONE)
			// Чё? Нахер это wpn_grenade_launcher когда уже есть дефолт wpn_launcher_def_bone ??? Небось пысозатычки от кривых конфигов или моделей...
			bone_id = pHudVisual->LL_BoneID(wpn_grenade_launcher);

		ASSERT_FMT_DBG(bone_id != BI_NONE, "[%s] invalid grenade launcher bone in section [%s]", __FUNCTION__,
		               this->cNameSect().c_str());
...
void CWeapon::reload(LPCSTR section)
{
	...
	if (m_eScopeStatus == ALife::eAddonAttachable)
	{
		m_addon_holder_range_modifier = READ_IF_EXISTS(pSettings, r_float, m_sScopeName, "holder_range_modifier",
		                                               m_holder_range_modifier);
		m_addon_holder_fov_modifier = READ_IF_EXISTS(pSettings, r_float, m_sScopeName, "holder_fov_modifier",
...
u8 CWeapon::GetCurrentHudOffsetIdx() const
{
	...
	bool b_aiming = ((IsZoomed() && /*m_zoom_params.*/m_fZoomRotationFactor <= 1.f) || (!IsZoomed() &&
...
{
	bool bCond_2 = m_fSecondVPZoomFactor > 0.0f;
	// В конфиге должен быть прописан фактор зума (scope_lense_fov_factor) больше чем 0
...
float CWeapon::GetHudFov()
{
	...
	if (m_fZoomRotationFactor > 0.0f)
	{
		...
		if ((m_eScopeStatus == CSE_ALifeItemWeapon::eAddonDisabled || IsScopeAttached()) && !IsGrenadeMode() &&
			m_fZoomHudFov > 0.0f)
		{
			// В процессе зума
