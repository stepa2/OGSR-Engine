...
{
	return std::find_if(m_calls.begin(), m_calls.end(),
	                    [&](CPHCall* call) { return call->equal(cmp_condition, cmp_action); });
}
...
CPHCall* CPHCommander::add_call_unique(CPHCondition* condition, CPHReqComparerV* cmp_condition, CPHAction* action,
