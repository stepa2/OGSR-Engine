...
void CElevatorState::UpdateStNone()
{
	...
	Fvector d;
	m_ladder->DToPlain(m_character, d);
	if (m_ladder->BeforeLadder(m_character) && m_ladder->InTouch(m_character) && dXZDotNormalized(
		d, m_character->CamDir()) > look_angle_cosine)
	{
...
bool CElevatorState::StateSwitchInertion(Estate new_state)
{
	...
	p.sub(m_start_position);
	if (m_etable[m_state][new_state].dist < p.magnitude() || m_etable[m_state][new_state].time < Device.dwTimeGlobal -
