...
void CStepManager::update()
{
	START_PROFILE("Step Manager")
		...
		for (u32 i = 0; i < m_legs_count; i++)
		{
			...
			if (offset_time <= cur_time)
			{
				...
				if (!mtl_pair->StepSounds.empty() && is_on_ground())
				{
					...
					sound_pos.y += 0.5;
					GET_RANDOM(mtl_pair->StepSounds).play_no_feedback(m_object, 0, 0, &sound_pos,
		...
		if (m_step_info.cur_cycle < step.cycles) m_step_info.cur_cycle = 1 + u8(
			float(cur_time - m_time_anim_started) / (1000.f * cycle_anim_time));
