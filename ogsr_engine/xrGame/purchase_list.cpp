...
void CPurchaseList::process(CInifile& ini_file, LPCSTR section, CInventoryOwner& owner)
{
	...
	if (pSettings->line_exist("engine_callbacks", "trade_purchase_item_process"))
	{
		...
		{
			Msg("Cannot get engine callback %s!",
			    pSettings->r_string("engine_callbacks", "trade_purchase_item_process"));
		}
	}
	...
	for (const auto& I : S.Data)
	{
		ASSERT_FMT(I.second.size(), "!![%s] : cannot handle lines in section without values: [%s], first param: [%s]",
		           __FUNCTION__, section, I.first.c_str());
		ASSERT_FMT(_GetItemCount(I.second.c_str()) > 1,
		           "!![%s] : Invalid second parameter in section [%s], first param: [%s]", __FUNCTION__, section,
		           I.first.c_str());
...
void CPurchaseList::process(const CGameObject& owner, const shared_str& name, const u32& count,
                            const float& probability, luabind::functor<void>& lua_function)
{
