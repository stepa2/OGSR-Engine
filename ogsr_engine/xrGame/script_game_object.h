...
public:
	...
	const CCoverPoint* best_cover(const Fvector& position, const Fvector& enemy_position, float radius,
	                              float min_enemy_distance, float max_enemy_distance, const luabind::functor<bool>&);
	const CCoverPoint* safe_cover(const Fvector& position, float radius, float min_distance);
	const CCoverPoint* safe_cover(const Fvector& position, float radius, float min_distance,
	                              const luabind::functor<bool>&);
	const CCoverPoint* ambush_cover(const Fvector& position, const Fvector& enemy_position, float radius,
	                                float min_distance);
	const CCoverPoint* ambush_cover(const Fvector& position, const Fvector& enemy_position, float radius,
	                                float min_distance, const luabind::functor<bool>&);
...
IC bool test_pushobject(lua_State* L, CGameObject* obj)
{
	...
	{
		convert_to_lua<T*>(L, pObj);
		// обязательно конвертировать указатель, а не значение. Иначе вызов деструктора при сборке мусора!
		return true;
	}
	return false;
}
