...
void CInventory::Take(CGameObject* pObj, bool bNotActivate, bool strict_placement)
{
	...
	case eItemPlaceRuck:
		...
		break;
	case eItemPlaceSlot:
		if (smart_cast<CActor*>(m_pOwner) && Device.dwPrecacheFrame && m_iActiveSlot == NO_ACTIVE_SLOT &&
			m_iNextActiveSlot == NO_ACTIVE_SLOT)
			bNotActivate = true;
...
bool CInventory::DropItem(CGameObject* pObj)
{
	...
	case eItemPlaceSlot:
		{
			ASSERT_FMT(InSlot(pIItem), "CInventory::DropItem: InSlot(pIItem): [%s], id: [%u]", pObj->cName().c_str(),
			           pObj->ID());
...
bool CInventory::Slot(PIItem pIItem, bool bNotActivate)
{
	...
	*/
	...
	if ((m_iActiveSlot == pIItem->GetSlot() || (m_iActiveSlot == NO_ACTIVE_SLOT && m_iNextActiveSlot == NO_ACTIVE_SLOT))
		&& !bNotActivate)
		Activate(pIItem->GetSlot());
