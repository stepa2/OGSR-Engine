...
void xrServer::Process_save(NET_Packet& P, ClientID sender)
{
	...
	while (!P.r_eof())
	{
		...
		{
			Msg(
				"!![%s] load/save mismatch, object: [%s], size: [%d], _pos_end-_pos_start: [%d], ID_to_entity(ID) is [%s]",
				__FUNCTION__, E ? E->name_replace() : "unknown", _size, _pos_end - _pos_start, E ? "true" : "false");
