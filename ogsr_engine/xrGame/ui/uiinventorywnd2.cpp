...
bool CUIInventoryWnd::OnItemSelected(CUICellItem* itm)
{
	...
	itm->ColorizeItems({
		m_pUIBagList, m_pUIBeltList, m_pUIPistolList, m_pUIAutomaticList, m_pUIKnifeList, m_pUIHelmetList,
		m_pUIBIODetList, m_pUINightVisionList, m_pUIDetectorList, m_pUITorchList, m_pUIBinocularList, m_pUIOutfitList
	});
	return false;
}
	...
	case iwSlot:
		{
			...
			if (item_w <= max_size.x && item_h <= max_size.y)
			{
					...
					if (new_owner == GetSlotList(i))
					{
						...
						else
						{
							if (!DropItem(CurrentIItem(), new_owner) && item->GetSlotsCount() > 0)
								Msg("! cannot put item %s into slot %d, allowed slots {%s}", name, i,
								    item->GetSlotsSect());
						}
						break;
					} // for-if
				...
				Msg("!#ERROR: item %s to large for slot: (%d x %d) vs (%d x %d) ", name, item_w, item_h, max_size.x,
				    max_size.y);
