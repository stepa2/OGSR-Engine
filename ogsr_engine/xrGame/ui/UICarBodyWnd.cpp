...
void CUICarBodyWnd::Update()
{
	...
	if (pOtherGO && pOurGO->Position().distance_to(pOtherGO->Position()) - pOtherGO->Radius() - pOurGO->Radius() >
		m_pOurObject->inventory().GetTakeDist() + 0.5f)
	{
...
void CUICarBodyWnd::Show()
{
	...
	if (Core.Features.test(xrCore::Feature::engine_ammo_repacker) && !Core.Features.test(
		xrCore::Feature::hard_ammo_reload))
		...
		if (smart_cast<CWeaponMagazined*>(pWeapon))
		{
			...
			if (!b)
			{
				...
				for (u32 i = 0; i < itm->ChildsCount(); ++i)
				{
					...
					if (pWeaponChild->GetAmmoElapsed() > 0 || (WpnMagazWglChild && !WpnMagazWglChild
					                                                                ->m_magazine2.empty()))
					{
						b = true;
						break;
...
void CUICarBodyWnd::EatItem()
{
	...
	if (owner_list == m_pUIOthersBagList)
	{
		u16 owner_id = (m_pInventoryBox)
			               ? m_pInventoryBox->object().ID()
			               : smart_cast<CGameObject*>(m_pOthersObject)->ID();
