...
void CUIWindow::DetachChild(CUIWindow* pChild, bool from_destructor)
{
	...
	{
		FATAL(
			"Exception catched in m_ChildWndList.remove(pChild)! Please send logs and minidumps to the engine developers!");
	}
...
bool CUIWindow::OnKeyboard(int dik, EUIMessages keyboard_action)
{
	...
	while (iter != m_ChildWndList.rend())
	{
		...
		ASSERT_FMT_DBG(Wnd, "!![%s][%s] Child wnd is nullptr! Something strange!", __FUNCTION__,
		               this->WindowName_script());
