...
void CUICellContainer::PlaceItemAtPos(CUICellItem* itm, Ivector2& cell_pos)
{
	...
	if (!m_pParentDragDropList->GetVirtualCells())
		itm->SetWndPos(Fvector2().set(((CellsSpacing().x + m_cellSize.x) * cp.x),
		                              ((CellsSpacing().y + m_cellSize.y) * cp.y)));
	else
	{
...
void CUICellContainer::GetTexUVLT(Fvector2& uv, u32 col, u32 row, u8 select_mode)
{
	...
	case 0: uv.set(0.00f, 0.0f);
		break;
	case 1: uv.set(0.125f, 0.0f);
		break;
	case 2: uv.set(0.250f, 0.0f);
		break;
	case 3: uv.set(0.375f, 0.0f);
		break;
	case 4: uv.set(0.500f, 0.0f);
		break;
	case 5: uv.set(0.625f, 0.0f);
		break;
	case 6: uv.set(0.750f, 0.0f);
		break;
	case 7: uv.set(0.875f, 0.0f);
		break;
