...
void CUITalkWnd::Update()
{
	...
	else
	{
		...
		if (NULL == pOurGO || NULL == pOtherGO || ((pOurGO->Position().distance_to(pOtherGO->Position()) - pOtherGO->
				Radius() - pOurGO->Radius() > m_pOurInvOwner->inventory().GetTakeDist() + 0.5f) && !m_pOthersInvOwner->
			NeedOsoznanieMode()))
			Game().StartStopMenu(this, true);
	}
...
void CUITalkWnd::AskQuestion()
{
	...
	if (TopicMode())
	{
		...
		{
			string128 s;
			sprintf_s(s, "ID = [%s] of selected question is out of range of available dialogs ",
