...
void CUIInventoryWnd::ActivatePropertiesBox()
{
			...
			if (slot != NO_ACTIVE_SLOT && slot != GRENADE_SLOT)
			{
				if (!m_pInv->m_slots[slot].m_pIItem || m_pInv->m_slots[slot].m_pIItem != CurrentIItem())
				{
					if (multi_slot && Core.Features.test(xrCore::Feature::slots_extend_menu))
					{
						...
						strconcat(sizeof(full_action_text), full_action_text, "st_move_to_slot_",
	...
	if (CurrentIItem()->Ruck() && m_pInv->CanPutInRuck(CurrentIItem()) && (CurrentIItem()->GetSlot() == NO_ACTIVE_SLOT
		|| !m_pInv->m_slots[CurrentIItem()->GetSlot()].m_bPersistent))
	{
		if (!pOutfit)
	...
	for (u8 i = 0; i < SLOTS_TOTAL; ++i)
	{
		...
		if (tgt && tgt->CanAttach(CurrentIItem()))
		{
			...
			std::snprintf(str, sizeof(str), CStringTable().translate(trans_str).c_str(),
			              CurrentIItem()->m_nameShort.c_str(), tgt->m_nameShort.c_str());
			std::string s(str);
