...
bool CUITradeWnd::CanMoveToOther(PIItem pItem, bool our)
{
	...
	if (pItem->GetCondition() < m_pOthersInvOwner->trade_parameters().factors(
		CTradeParameters::action_buy(0),
		pItem->object().cNameSect()).min_condition())
		return false;
...
bool CUITradeWnd::OnItemSelected(CUICellItem* itm)
{
	SetCurrentItem(itm);
	itm->ColorizeItems({
		&m_uidata->UIOurTradeList, &m_uidata->UIOthersTradeList, &m_uidata->UIOurBagList, &m_uidata->UIOthersBagList
	});
	return false;
}
