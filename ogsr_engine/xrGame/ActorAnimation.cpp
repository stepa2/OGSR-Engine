...
void CActor::g_SetAnimation(u32 mstate_rl)
{
				...
				if (!m_bAnimTorsoPlayed)
				{
					if (W)
					{
						...
						else
						{
							switch (W->GetState())
							{
							case CWeapon::eIdle: M_torso = W->IsZoomed() ? TW->zoom : TW->moving[moving_idx];
								break;
					...
					else if (M)
					{
						if (is_standing)
						{
							switch (M->GetState())
							{
							case MS_SHOWING: M_torso = TW->draw;
								break;
							case MS_HIDING: M_torso = TW->holster;
								break;
							case MS_IDLE: M_torso = TW->moving[moving_idx];
								break;
							case MS_EMPTY: M_torso = TW->zoom;
								break;
							case MS_THREATEN: M_torso = M_legs = M_head = TW->all_attack_0;
								break;
							case MS_READY: M_torso = M_legs = M_head = TW->all_attack_1;
								break;
							case MS_THROW: M_torso = M_legs = M_head = TW->all_attack_2;
								break;
							case MS_END: M_torso = M_legs = M_head = TW->all_attack_2;
								break;
						...
						else
						{
							...
							case MS_IDLE: M_torso = TW->moving[moving_idx];
								break;
							case MS_EMPTY: M_torso = TW->moving[moving_idx];
								break;
							case MS_THREATEN: M_torso = TW->attack_zoom;
								break;
							case MS_READY: M_torso = TW->fire_idle;
								break;
							case MS_THROW: M_torso = TW->fire_end;
								break;
							case MS_END: M_torso = TW->fire_end;
								break;
							default: M_torso = TW->draw;
								break;
							}
	...
	{
		if (m_bAnimTorsoPlayed) m_current_torso_blend = smart_cast<IKinematicsAnimated*>(Visual())->PlayCycle(
	...
	if (m_current_legs != M_legs)
	{
		...
		if ((mstate_real & mcAnyMove) && (mstate_old & mcAnyMove) && m_current_legs_blend)
			pos = fmod(m_current_legs_blend->timeCurrent, m_current_legs_blend->timeTotal) / m_current_legs_blend->
				timeTotal;
		m_current_legs_blend = smart_cast<IKinematicsAnimated*>(Visual())->PlayCycle(
			M_legs,TRUE, legs_play_callback, this);
