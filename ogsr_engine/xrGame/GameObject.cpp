	...
	if (visual)
	{
		...
		if (pSettings->line_exist(cNameSect(), "visual"))
		{
			...
			if (keep_visual)
			{
				...
				{
					Msg("! [%s]: changed visual_name[%s] found in %s, keep original %s instead", __FUNCTION__,
					    saved_visual, cName().c_str(), config_visual.c_str());
				}
			}
			else if (!FS.exist(saved_visual) && !FS.exist("$level$", saved_visual_file) && !FS.exist(
				"$game_meshes$", saved_visual_file))
			{
				Msg("! [%s]: visual_name[%s] not found in %s, keep original %s instead", __FUNCTION__, saved_visual,
				    cName().c_str(), config_visual.c_str());
			}
			else
	...
	else
	{
		if (ai().get_level_graph())
		{
			...
			if (!_valid(Position()))
			{
				...
				Msg("! [%s]: %s has invalid Position[%f,%f,%f] level_vertex_id[%u][%f,%f,%f]", __FUNCTION__,
				    cName().c_str(), Position().x, Position().y, Position().z, ai_location().level_vertex_id(),
...
CScriptCallbackEx<void>& CGameObject::callback(GameObject::ECallbackType type) const
{
	if (auto it = m_callbacks.find(type); it != m_callbacks.end())
		...
		//KRodin: вот по-нормальному, надо возвращать nullptr и проверять в вызывающей функции, есть ли каллбек вообще. Но это кучу функций переделать надо, поэтому буду возвращать фейковый каллбек.
...
void CGameObject::addFeelTouch(float radius, const luabind::object& lua_object,
...
void CGameObject::removeFeelTouch(const luabind::object& lua_object, const luabind::functor<void>& new_delete,
