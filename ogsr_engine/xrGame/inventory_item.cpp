...
CInventoryItem::~CInventoryItem()
{
	...
	ASSERT_FMT((int)m_slots.size() >= 0, "m_slots.size() returned negative value inside destructor!");
...
void CInventoryItem::SetSlot(u8 slot)
{
	...
	{
		Msg("!#ERROR: slot %d not acceptable for object %s (%s) with slots {%s}", slot, object().Name_script(), Name(),
		    m_slots_sect);
		return;
	}
}
...
void CInventoryItem::CalculateInterpolationParams()
{
	...
	if (m_flags.test(FInInterpolation))
	{
		...
		for (u32 k = 0; k < 3; k++)
		{
			...
			P1[k] = (c * c * p->SCoeff[k][0] * 3 + c * p->SCoeff[k][1] * 2 + p->SCoeff[k][2]) / 3;
			// сокрость из формулы в 3 раза превышает скорость при расчете коэффициентов !!!!
		};
		P0.set(p->IStartPos);
		P1.add(p->IStartPos);
	}
	...
	u32 ConstTime = u32((fixed_step - ph_world->m_frame_time) * 1000) + Level().GetInterpolationSteps() * u32(
		fixed_step * 1000);
