...
void CChimera::Load(LPCSTR section)
{
	...
	{
		m_velocity_rotate.Load(section, "Velocity_Stand");
		m_velocity_jump_start.Load(section, "Velocity_JumpGround");
	}
	...
	else
	{
		...
		anim().AddAnim(eAnimFastStandTurnLeft, "stand_turn_ls_", -1, &m_velocity_rotate, PS_STAND);
		anim().AddAnim(eAnimFastStandTurnRight, "stand_turn_ls_", -1, &m_velocity_rotate, PS_STAND);
		...
		anim().AddAnim(eAnimRunTurnLeft, "stand_run_fwd_", -1, &velocity_run, PS_STAND);
		anim().AddAnim(eAnimRunTurnRight, "stand_run_fwd_", -1, &velocity_run, PS_STAND);
	}
