...
void CPoltergeist::Load(LPCSTR section)
{
	...
	m_fly_around_distance = READ_IF_EXISTS(pSettings, r_float, section, "detection_fly_around_distance",
	                                       strcmp(polter_type, "flamer") == 0 ? 8.f : 30.f);
...
void CPoltergeist::update_detection()
{
	...
	Actor()->SetActorVisibility(ID(), get_post_process_factor());
