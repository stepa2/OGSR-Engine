...
void CControlAnimationBase::select_animation(bool anim_end)
{
	...
	MotionID cur_anim = smart_cast<IKinematicsAnimated*>(m_object->Visual())->ID_Cycle_Safe(
		xr_strconcat(s2, *anim_it->target_name, itoa(index, s1, 10)));
	if (!cur_anim.valid())
		FATAL(s2);
...
{
	// найти анимацию 
	ASSERT_FMT(u32(a) < m_anim_storage.size(), "[%s]: %s: a[%u] m_anim_storage.size[%u]", __FUNCTION__,
	           m_object->cName().c_str(), a, m_anim_storage.size());
	SAnimItem* item_it = m_anim_storage[a];
	ASSERT_FMT(item_it, "[%s]: %s: m_anim_storage[%u] is NULL", __FUNCTION__, m_object->cName().c_str(), a);
	//VERIFY(item_it);
...
CMotionDef* CControlAnimationBase::get_motion_def(SAnimItem* it, u32 index)
{
	...
	const MotionID& motion_id = skeleton_animated->ID_Cycle_Safe(
		xr_strconcat(s2, *it->target_name, itoa(index, s1, 10)));
