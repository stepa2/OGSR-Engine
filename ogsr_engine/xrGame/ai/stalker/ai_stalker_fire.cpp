...
void CAI_Stalker::Hit(SHit* pHDS)
{
	...
	if (m_boneHitProtection && HDS.hit_type == ALife::eHitTypeFireWound)
	{
		...
		if (NewHitPower < HDS.power * m_boneHitProtection->m_fHitFrac) HDS.power = HDS.power * m_boneHitProtection->
			m_fHitFrac;
		else
	...
	if (g_Alive())
	{
		...
		if (!already_critically_wounded)
		{
			...
			if (cover && pHDS->initiator() && (pHDS->initiator()->ID() != ID()) && !fis_zero(pHDS->damage()) && brain().
				affect_cover())
	...
	// try to find the best item which can kill
	{
		...
		for (; I != E; ++I)
		{
			if ((*I)->can_kill())
			{
				...
				if (m_best_item_to_kill && ((*I)->object().ef_weapon_type() <= m_best_item_to_kill
				                                                               ->object().ef_weapon_type()))
					continue;
...
static void throw_position(Fvector& result, const Fvector& start_position, const Fvector& velocity,
