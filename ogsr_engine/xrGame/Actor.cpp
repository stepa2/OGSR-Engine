...
#pragma todo("Dima to Dima : forward-back bone impulse direction has been determined incorrectly!")
		MotionID motion_ID = m_anims->m_normal.m_damage[iFloor(
			pK->LL_GetBoneInstance(element).get_param(1) + (angle_difference(r_model_yaw + r_model_yaw_delta,yaw) <=
				PI_DIV_2 ? 0 : 1))];
...
void CActor::g_Physics(Fvector& _accel, float jump, float dt)
{
	...
	if (Local() && g_Alive())
	{
		...
		if (!fis_zero(character_physics_support()->movement()->gcontact_HealthLost))
		{
			...
			{
				SHit HDS = SHit(character_physics_support()->movement()->gcontact_HealthLost, hdir,
				                di->DamageInitiator(), character_physics_support()->movement()->ContactBone(),
...
void CActor::ForceTransform(const Fmatrix& m, const bool from_demo_record)
{
	...
	if (character_physics_support()->movement()->CharacterExist()) character_physics_support()
...
{
	float k = (Core.Features.test(xrCore::Feature::af_zero_condition) && fis_zero(artefact->GetCondition()))
		          ? 0.f
		          : 1.f;
...
ActorRestoreParams CActor::ActiveArtefactsOnBelt()
{
	...
	if (Core.Features.test(xrCore::Feature::objects_radioactive))
	{
		...
		for (TIItemContainer::iterator it = map_all.begin(); map_all.end() != it; ++it)
		{
			...
			float k = (Core.Features.test(xrCore::Feature::af_zero_condition) && fis_zero(obj->GetCondition()))
				          ? 0.f
				          : 1.f;
	...
	if (Core.Features.test(xrCore::Feature::outfit_af))
	{
		...
		if (outfit_item)
		{
			...
			if (outfit)
			{
				float k = (Core.Features.test(xrCore::Feature::af_zero_condition) && fis_zero(outfit->GetCondition()))
					          ? 0.f
					          : 1.f;
...
{
	return mstate_real & (mcJump | mcFall | mcLanding | mcLanding2 | mcLookout | mcCrouch | mcAccel | mcClimb |
		       mcLStrafe | mcRStrafe)
		       ? false
...
void CActor::RepackAmmo()
{
	...
	while (!_ammo.empty())
	{
		...
		_ammo.erase(std::remove_if(_ammo.begin(), _ammo.end(), [asect](CWeaponAmmo* a)
		{
			return a->cNameSect() == asect;
		}), _ammo.end());
	}
}
