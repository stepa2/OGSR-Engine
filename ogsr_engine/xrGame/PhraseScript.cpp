...
bool CPhraseScript::Precondition(const CGameObject* pSpeakerGO, LPCSTR dialog_id, LPCSTR phrase_id) const
{
	...
	for (const auto& Cond : Preconditions())
	{
		...
		if (luabind::functor<bool> lua_function; ai().script_engine().functor(ConditionString.c_str(), lua_function))
			// Обычный функтор
		...
		else
		{
			...
			luabind::object ret_obj = loadstring_functor(ConditionString.c_str());
			// Создаём функцию из строки через loadstring
			auto ret_func = luabind::object_cast<luabind::functor<bool>>(ret_obj);
			// Первое возвращённое loadstring значение должно быть функцией
			ASSERT_FMT_DBG(ret_func, "Loadstring returns nil for code: %s", ConditionString.c_str());
...
void CPhraseScript::Action(const CGameObject* pSpeakerGO, LPCSTR dialog_id, LPCSTR /*phrase_id*/) const
{
	for (const auto& Act : Actions())
	{
		...
		if (luabind::functor<void> lua_function; ai().script_engine().functor(ActionString.c_str(), lua_function))
			// Обычный функтор
		...
		else
		{
			...
			luabind::object ret_obj = loadstring_functor(ActionString.c_str());
			// Создаём функцию из строки через loadstring
			auto ret_func = luabind::object_cast<luabind::functor<void>>(ret_obj);
			// Первое возвращённое loadstring значение должно быть функцией
			ASSERT_FMT_DBG(ret_func, "Loadstring returns nil for code: %s", ActionString.c_str());
			// Если это не функция, значит loadstring вернул nil и что-то пошло не так
			// Вызываем созданную функцию и передаём ей дефолтные аргументы. Они прилетят после аргументов, прописанных явно, если например сделать так:
                                 ...
                                 LPCSTR phrase_id, LPCSTR next_phrase_id) const
{
	...
	for (const auto& Cond : Preconditions())
	{
		...
		else
		{
			...
			// А если не указать '...' - дефолтные аргументы не будут переданы в функцию.
			if (ret_func)
				predicate_result = ret_func(pSpeakerGO1->lua_game_object(), pSpeakerGO2->lua_game_object(), dialog_id,
				                            phrase_id, next_phrase_id);
		}
...
LPCSTR CPhraseScript::GetScriptText(LPCSTR str_to_translate, const CGameObject* pSpeakerGO1,
                                    const CGameObject* pSpeakerGO2, LPCSTR dialog_id, LPCSTR phrase_id)
{
	...
	if (luabind::functor<const char*> lua_function; ai().script_engine().functor(ScriptTextString.c_str(), lua_function)
	) // Обычный функтор
	...
	else
	{
		...
		luabind::object ret_obj = loadstring_functor(ScriptTextString.c_str());
		// Создаём функцию из строки через loadstring
		auto ret_func = luabind::object_cast<luabind::functor<const char*>>(ret_obj);
		// Первое возвращённое loadstring значение должно быть функцией
		ASSERT_FMT_DBG(ret_func, "Loadstring returns nil for code: %s", ScriptTextString.c_str());
		// Если это не функция, значит loadstring вернул nil и что-то пошло не так
		// Вызываем созданную функцию и передаём ей дефолтные аргументы. Они прилетят после аргументов, прописанных явно, если например сделать так:
