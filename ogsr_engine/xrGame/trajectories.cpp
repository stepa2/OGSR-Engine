...
static float trajectory_pick_error(float low, float high, const Fvector& position, const Fvector& velocity,
                                ...
                                xr_vector<Fvector>* const out_collide_tris)
{
	...
	if (box_size.magnitude() <= epsilon)
		Level().ObjectSpace.RayQuery(temp_rq_results, ray_defs, (collide::rq_callback*)trajectory_query_callback,
		                             &range, nullptr, self_object);
	else
	{
		Fvector box_center;
