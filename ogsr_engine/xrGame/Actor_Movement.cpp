...
void CActor::g_cl_ValidateMState(float dt, u32 mstate_wf)
{
	...
	if (((character_physics_support()->movement()->GetVelocityActual() < 0.2f) && (!(mstate_real & (mcFall | mcJump))))
		...
		character_physics_support()->movement()->Environment() == CPHMovementControl::peAtWall)
	{
		...
		if ((mstate_real & mcCrouch) == 0 && (mstate_wf & mcCrouch))
		{
			...
			else
			{
				...
				if (isActorAccelerated(mstate_wf, IsZoomAimingMode()))
					Crouched = character_physics_support()->movement()->ActivateBoxDynamic(
						(mstate_wf & mcAnyMove) ? 3 : 1);
				else
					Crouched = character_physics_support()->movement()->ActivateBoxDynamic(
						(mstate_wf & mcAnyMove) ? 4 : 2);
				if (Crouched)
					mstate_real |= mcCrouch;
			}
		}
		...
		*/
		...
		if (mstate_real & mcCrouch)
		{
			...
			else if (isActorAccelerated(mstate_real, IsZoomAimingMode()) && !isActorAccelerated(
				mstate_wf, IsZoomAimingMode()))
			{
				character_physics_support()->movement()->EnableCharacter();
			...
			{
				character_physics_support()->movement()->EnableCharacter();
				character_physics_support()->movement()->ActivateBoxDynamic(
					isActorAccelerated(mstate_wf, IsZoomAimingMode()) ? 1 : 2);
				moving_box_delay = 0;
				crouch_move = false;
				crouch_stop = false;
			}
...
void CActor::g_Orientate(u32 mstate_rl, float dt)
{
	...
	{
		r_torso_tgt_roll = angle_inertion_var(r_torso_tgt_roll, tgt_roll, 0.f,
		                                      CurrentHeight * PI_MUL_2 * cam_LookoutSpeed, PI_DIV_2, dt);
...
void CActor::g_cl_Orientate(u32 mstate_rl, float dt)
{
	...
	CWeaponMagazined* pWM = smart_cast<CWeaponMagazined*>(inventory().GetActiveSlot() != NO_ACTIVE_SLOT
		                                                      ...
		                                                      /*inventory().m_slots[inventory().GetActiveSlot()].m_pIItem*/
		                                                      : NULL);
