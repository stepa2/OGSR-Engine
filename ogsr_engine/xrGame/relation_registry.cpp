		...
		if (it.first)
		{
			...
			for (const auto& it2 : relation_data.personal)
			{
				if (it2.first)
				{
					...
					const auto it3 = std::find(relation_data2.reverse_personal.begin(),
					                           relation_data2.reverse_personal.end(), it.first);
					ASSERT_FMT(it3 == relation_data2.reverse_personal.end(),
					           "[%s]: %u already exists in revers_personal of %u", __FUNCTION__, it.first, it2.first);
...
void RELATION_REGISTRY::ClearRelations(u16 person_id)
{
	if (relation_registry().registry().objects_ptr(person_id))
	{
		...
		{
			ASSERT_FMT(relation_registry().registry().objects_ptr( to ), "[%s]: %u not found clearing %u", __FUNCTION__,
			           to, person_id);
			auto& relation_data2 = relation_registry().registry().objects(to);
			const auto it = relation_data2.personal.find(person_id);
			ASSERT_FMT(it != relation_data2.personal.end(), "[%s]: %u not found in personal of %u", __FUNCTION__,
			           person_id, to);
...
void RELATION_REGISTRY::clear_reverse_personal(u16 from, u16 to)
{
	...
	ASSERT_FMT(relation_registry().registry().objects_ptr( to ), "[%s]: %u not found clearing %u", __FUNCTION__, to,
	           from);
	...
	ASSERT_FMT(it != relation_data.reverse_personal.end(), "[%s]: %u not found in reverse_personal of %u", __FUNCTION__,
	           from, to);
